  ...
  return (
    <ThemedView style={[styles.screen, { backgroundColor: pageBg }]}>
      <View style={styles.header}>
        ...
        <ThemedText style={styles.bigTitle}>灵感碎片（列表）</ThemedText>
        <View style={{ width: 40 }} />
      </View>

      <View style={[styles.card, { backgroundColor: cardBg, borderColor: cardBorder }]}>
        <ThemedText style={styles.cardTitle}>最近记录</ThemedText>
        <ThemedText style={[styles.subtitle, { color: mutedText }]}>还没灵感？那就先活着。</ThemedText>
      </View>

      <Pressable
        onPress={() => {}}
        style={({ pressed }) => [styles.primaryBtn, { backgroundColor: accent, opacity: pressed ? 0.92 : 1 }]}>
        <ThemedText style={styles.primaryText}>+ 记一条灵感</ThemedText>
      </Pressable>
    </ThemedView>
  );
}
